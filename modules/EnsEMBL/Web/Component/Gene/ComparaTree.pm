...
# EG include the ht param
  my $default_view_url = $hub->url({ $hub->param('ht') ? (ht => $hub->param('ht')) : (), collapse => $collapsed_to_gene, g1 => $highlight_gene });
  ...
  if ($gene) {
    ...
    push @view_links, sprintf $li_tmpl, $hub->url({ $hub->param('ht') ? (ht => $hub->param('ht')) : (), collapse => $collapsed_to_para || undef, g1 => $highlight_gene }), $highlight_gene ? 'View paralogs of current genes' : 'View paralogs of current gene';
  }
  
  push @view_links, sprintf $li_tmpl, $hub->url({ $hub->param('ht') ? (ht => $hub->param('ht')) : (), collapse => $collapsed_to_dups, g1 => $highlight_gene }), 'View all duplication nodes';
  push @view_links, sprintf $li_tmpl, $hub->url({ $hub->param('ht') ? (ht => $hub->param('ht')) : (), collapse => 'none', g1 => $highlight_gene }), 'View fully expanded tree';
